 ...
 */
const writeModuleToModulesJSON = (name, tag, provider, exportDefault, url, callback) => {
	let spinner = ora({
	...
	fs.readFile(path.resolve('modules.json'), 'utf8', (err, data) => {
		...
		if (!exportDefault) {
			modules[name].default = exportDefault;
		}
			...
			if (options.check) {
				...
				https.get(moduleUrl, res => { 
					if (res.statusCode === 200) {
						...
						writeModuleToModulesJSON(name, options.tag, options.provider, options.default, options.url, err => {
							if (!err)
